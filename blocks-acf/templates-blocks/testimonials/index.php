<?php
/**
 * Testimonials ACF Custom Block Render
 *
 * @package WordPress
...
if ( have_rows( 'testimonials' ) ) : ?>
	<section id="<?php echo esc_attr( $anchor ); ?>" class="testimonials grid grid-cols-1 md:grid-cols-3 gap-5 w-full px-5 py-5">
		...
		while ( have_rows( 'testimonials' ) ) :
			...
			$text        = get_sub_field( 'testomonial_text' ) ? get_sub_field( 'testomonial_text' ) : 'Your testimonial here...';
			$author      = get_sub_field( 'author_name' ) ? get_sub_field( 'author_name' ) : 'Author name';
			$author_role = get_sub_field( 'author_role' ) ? get_sub_field( 'author_role' ) : 'Author role';
			$image       = get_sub_field( 'author_image' ) ? get_sub_field( 'author_image' ) : 295;
